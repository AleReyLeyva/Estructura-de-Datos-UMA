...
p2_ordena3 x y z = mismosElementos (x,y,z) (ordena3 (x,y,z))
    where mismosElementos (x,y,z) (j,k,l) = (x==j && y==k && z==l) || (x==k && y==l && z==j) || (x==l && y==j && z==k) || (x==j && y==l && z==k) || (x==k && y==j && z==l) || (x==l && y==k && z==j)
...
p1_max2 x y = biendef (max2 x y)
    where biendef a = a==x || a==y

p2_max2 x y = mayorigual (max2 x y)
    where mayorigual a = a>=x && a>=y
...
raíces a b c | ((b^2 - (4*a*c)) < 0) = error "Polinomio sin raíces reales"
...
potencia x y | y < 0 = error "El exponente debe ser positivo"
             | y == 0 = 1
...
factorial x | x<0 = error "El número debe ser mayor o igual q cero"
            | x==0 = 1
					  | otherwise = x * factorial (x-1)

-- 16.a
divideA :: Integer -> Integer -> Bool
divideA x y | x/=0 = mod y x == 0
