...
get :: Int -> Vector a -> a
get i (V n (Unif a))
  | (i>=n) || (i<0) = error "Index Out of Bounds Exception"
  | otherwise = a
get i (V n (Node lt rt))
  | (i>=n) || (i<0) = error "Index Out of Bounds Exception"
  | otherwise = if i <= (div n 2 - 1) then get i (V (div n 2) lt) else get (i - div n 2) (V (div n 2) rt)
...
set i x (V n (Unif y))
  | (i>=n) || (i<0) = error "Index Out of Bounds Exception"
  | n == 1 = V n (Unif x)
  | otherwise = if i <= (div n 2 - 1)
    then V n (simplify (getTree (set i x (V (div n 2) (Unif y)))) (Unif y)) 
    else V n (simplify (Unif y) (getTree (set (i - div n 2) x (V (div n 2) (Unif y)))))
set i x (V n (Node lt rt))
  | (i>=n) || (i<0) = error "Index Out of Bounds Exception"
  | i <= (div n 2 - 1) = V n (simplify (getTree (set i x (V (div n 2) lt))) rt)
  | otherwise = V n (simplify lt (getTree (set (i - div n 2) x (V (div n 2) rt))))

getTree :: Vector a -> Tree a
getTree (V n x) = x
...
mapVector f (V n (Unif x)) = V n (Unif (f x))
mapVector f (V n (Node lt rt)) = V n (simplify (getTree (mapVector f (V (div n 2) lt))) (getTree (mapVector f (V (div n 2) rt)))) 
